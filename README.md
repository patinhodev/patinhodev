...
<style>
  ...
  canvas {
    image-rendering: pixelated; /* Dá o aspecto Pixelado perfeito do Atari */
    box-shadow: 0 0 20px rgba(0,0,0,0.8);
    border: 4px solid #333;
    width: 100%;
    max-width: 500px; /* Tamanho final de tela */
  }
</style>
</head>
<body>
...
<script>
  ...
  function update() {
    ...
    // Movimentar e desenhar o rio de forma dinâmica (Ilusão de voo)
    if(frame % 2 === 0) {
    ...
    // Piloto Automático manobrando o avião
    ...
    // Inimigos subindo a tela e verificando morte
    enemies.forEach(en => {
        if(en.active) {
            en.y += 2.2; // Velocidade da descida
        ...
        // Fazer respawn ciclico dos inimigos
        if(en.y > 450 || (!en.active && en.y > en.y + 80)) {
            en.y = -80 - Math.random() * 150;
            en.x = leftBank[0] + 30 + Math.random() * 80; // Manter no meio da água
            en.active = true;
        }
    });
